...
- 所有已实现及后续前端页面以本机 `emby-win` 对应源码为视觉与交互基线；
  详情页统一采用 75vh/650px 英雄图、130px 内容覆盖和沉浸式渐变顶栏，
  电影与电视剧不得维护两套互相偏离的信息架构。
...
- 季选择器。
- 单集列表、缩略图、简介、时长和进度。
- 单集详情复用所属剧集视觉与内容层级，单集简介优先替换剧集简介；本季单集、
  演职人员和相关推荐继续保留。
- 页面末尾以 Emby 风格分栏展示安全的媒体源、视频、音频和字幕流格式字段，
  不返回路径、直连地址、Token 或上游鉴权头。
- “播放下一未看集”和“从第一集开始”。
- 季级别收藏与已看操作。
...
- 首选音频语言和字幕语言。
- 自动选择字幕、强制字幕策略。
- 暗黑、亮色、跟随系统三种主题已提前由 `UX-007` 提供；设置页继续提供同一
  主题选择，并在 M3-009 补充密度、动效和海报尺寸。
- Bridge 配对设备列表和解除配对。
