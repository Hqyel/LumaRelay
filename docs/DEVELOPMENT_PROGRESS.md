...
### 6.3.2 全站视觉复原与主题（独立计数：2/2）

本轮继续逐页对照 `emby-win` 的实际组件源码补齐控件、间距、层级、
玻璃质感和动效，不以既有“已迁移”状态代替细节验收。

- [x] `UX-007` 亮色、暗黑、跟随系统主题基础与自定义控件复原。
- [x] `UX-008` 已实现页面逐页复原与全量主题视觉、键盘和可访问性回归。
...
- [x] `M2-QA-002` 核验电影进度持久化并完善单集详情与视频格式信息。
  - 状态：完成。
  - 区分播放事件链故障与 Emby 媒体库最小续播百分比策略。
  - 单集详情复用剧集详情的沉浸式结构，英雄区优先以单集简介替换剧集简介，
    不再重复显示单集摘要卡。
  - 页面末尾按 Emby 风格展示安全媒体源摘要及视频、音频和字幕流详情；路径、
    直连地址、Token 与请求头不进入浏览器。
...
- 当前无新的代码任务进行中；等待真实 PotPlayer 播放进度发布门复测。
...
1. 使用全新的播放会话确认 Emby 后台在播放期间显示活动影片并产生 Playing/
   Stopped 记录；播放电影直至超过对应媒体库的最小续播百分比，正常退出后验收
   续播位置误差不超过 15 秒。
2. 真实播放发布门通过后开始 `M3-001`；M3-009 只补设置页入口、界面密度、
   动效和海报尺寸，三种主题基础不再重复实现。
3. 安装 .NET SDK 8.0.422 后恢复包含 Bridge 的 `verify:local` 全量校验。
4. 配置 GitHub 远程并让 Actions 首次全量通过，完成 `M0-004`。
5. 安装 Docker 后实际构建并启动 Compose/Caddy 示例。
...
|---|---|---|---|---|---|
| 2026-07-23 | UX-008 | 完成 | 已逐页对照 `emby-win` 收口连接、登录、首页、媒体库、搜索、管理入口、播放准备及电影/电视剧/单集详情，并完成暗黑、亮色、跟随系统主题。三类详情统一使用 75vh 参考英雄层级、精确元数据、常驻版本/音轨/字幕、演员、相关推荐与 Emby 风格媒体信息；顶栏在普通页面保持实体主题背景，沉浸首屏与英雄图融合且滚动后恢复实体背景，活动导航移除额外浅色光晕。剧情简介和单集简介在同行省略号后提供“更多”弹层；超长版本值仅在控件 Hover 时循环滚动，单集标题由整张卡片 Hover 触发循环且正文背景始终透明，移开均复位，键盘保留 `Shift + ←/→`。超长版本播放弹窗已补齐 `minmax(0, …)` 收缩边界。除本机缺少 .NET SDK 8.0.422 的 Bridge 复跑外，6 个 JS/TS 工作区 lint/typecheck/build、255 项单测、Storybook 构建、2 项组件视觉/axe、32 项 Chromium E2E（1 项按设计跳过）、Chromium/Firefox 2 项兼容回归、本地 Smoke、生产依赖审计、格式与差异检查全部通过；暗黑/亮色详情及单集视觉基线已人工核对 | UI、Web、Playwright、视觉基线、UX 规范、进度表 | 真实 PotPlayer 播放进度发布门；通过后开始 M3-001 |
| 2026-07-23 | UX-008 | 进行中 | 用户实测确认单集详情仍使用旧版 `MediaDetailHero`，且沉浸顶栏底部残留 1px 黑线。单集现已改为直接复用电影/电视剧的参考英雄组件，仅用当前单集覆盖季集号、简介、播放目标和媒体源；本季单集、相关演员、相关推荐、媒体信息保持固定顺序。沉浸顶栏首屏已彻底移除边框宽度，仅在滚动转为实色后恢复分隔。Web 37 项单测及 lint/typecheck 通过，单集 axe/视觉回归 1/1 通过且基线已人工核对，正在执行最终全量回归 | Web、UI、Playwright、UX 规范、进度表 | 全量 Chromium、兼容、构建、Smoke 与审计通过后完成 UX-008 |
| 2026-07-22 | UX-008 | 进行中 | 已直接按参考 `DetailView.vue` 将电影与电视剧统一为 75vh/最大 650px 背景、底部透明渐隐、内容上移 130px、标题/精确元数据/技术徽标、播放与简介并排、常驻版本/音轨/字幕、相关演员/相关推荐/媒体信息结构；电视剧英雄区由当前续播或未看单集驱动季集号、时长和媒体规格。详情顶栏改为叠图渐变，滚动 96px 后平滑恢复主题实色毛玻璃。UI 26 项、Web 37 项单测及两包 lint/typecheck 已通过，电影暗黑/亮色和电视剧截图回归 3/3 通过并已人工核对，正在执行全量回归 | UI、Web、Playwright、UX/项目文档、进度表 | 全量 Chromium、兼容、构建、Storybook、Smoke 与审计通过后完成 UX-008 |
| 2026-07-22 | UX-008 | 验收返修 | 第一轮三主题与共享控件质量门已通过，但用户使用同一真实电影并排对照后确认详情页仍是旧版纵向信息架构：Logo、元数据、操作和简介堆叠，缺少参考应用的标题/技术标签、播放与简介并排以及常驻版本/音轨/字幕区。任务恢复为进行中，正在直接按 `DetailView.vue` 模板和 CSS 返修，不沿用此前完成判断 | Web、参考截图、`DetailView.vue`、进度表 | 完成电影详情同源复原后重新执行全量回归 |
