...
- [ ] `M1-013` 首页 `/home`。
  - 包含焦点区、继续观看、最近电影和最近剧集。
- [ ] `M1-014` 电影库 `/movies`。
- [ ] `M1-015` 剧集库 `/series`。
- [ ] `M1-016` 通用媒体库 `/library/:libraryId`。
- [ ] `M1-017` 搜索 `/search`。
- [ ] `M1-018` 电影详情 `/item/:id`。
...
- 本机未安装 Docker，Compose/Caddy 示例尚未实际构建和启动，相关 M0
  发布门保持未勾选。
- 尚未提供目标 Emby 地址，公共服务器探测仅通过本地模拟服务验证，目标
  版本 smoke test 发布门保持未勾选。

### 建议下一步

1. 完成 `QA-003`，导出领域适配器、修正延迟语义并建立真实 Emby Smoke。
2. 完成 `QA-004` 后再开始 `M1-013`。
