...
- `UX-008`：正在逐页清理暗色硬编码，对照 `emby-win` 复原卡片、
  页面层级、间距、玻璃质感和动效，并执行三主题视觉与交互回归。
...
1. 完成 `UX-007` 后逐页执行 `UX-008` 视觉复原与三主题回归。
2. 使用全新的播放会话确认 Emby 后台在播放期间显示活动影片并产生 Playing/
   Stopped 记录；播放电影直至超过对应媒体库的最小续播百分比，正常退出后验收
   续播位置误差不超过 15 秒。
3. 真实播放发布门通过后开始 `M3-001`。
