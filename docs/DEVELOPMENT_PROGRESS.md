...
| M1 | 媒体浏览 MVP | 进行中 | 27/28 | 带 M0 外部阻塞进入 |
...
- `M1-028` 进行中：验证 Chrome、Edge 和 Firefox 桌面兼容性。
...
1. 完成 `M1-028` Chrome、Edge、Firefox 桌面兼容验证。
2. M1 发布门全部满足后进入 `M2-001` Player Bridge 工程初始化。
